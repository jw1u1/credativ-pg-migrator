    ...
    def run_prepare_data_sources(self):
        ...
        if database_export['format'] in ('CSV', 'UNL'):
            for table in self.migrator_tables.fetch_all_tables():
                self.config_parser.print_log_message('DEBUG', f"Planner: run_prepare_data_sources: run_prepare_data_sources: Processing table: {table}")
                ...
                if table_database_export:
                    ...
                    self.config_parser.print_log_message('DEBUG', f"Planner: run_prepare_data_sources: run_prepare_data_sources: Table {table_info['source_table_name']} has specific database export settings: {table_database_export}")
                ...
                if file_name:
                    ...
                    if os.path.exists(table_file_name):
                        ...
                        self.config_parser.print_log_message('ERROR', f"Planner: run_prepare_data_sources: run_prepare_data_sources: Data source file {table_file_name} does not exist or is not accessible.")
                        data_file_found = False
                        if self.config_parser.get_source_database_export_on_missing_data_file() == 'error':
                            self.config_parser.print_log_message('ERROR', f"Planner: run_prepare_data_sources: run_prepare_data_sources: Data source file {table_file_name} does not exist or is not accessible. Stopping execution.")
                            exit(1)
                    ...
                    self.config_parser.print_log_message('DEBUG3',f"Planner: run_prepare_data_sources: run_prepare_data_sources: Table {table_info['source_table_name']} - file_name: {table_file_name}, converted_file_name: {converted_file_name}, data_file_found: {data_file_found}, format: {format}, delimiter: {delimiter}, header: {header}")
                    data_source = {
                    ...
                    self.config_parser.print_log_message('DEBUG', f"Planner: run_prepare_data_sources: run_prepare_data_sources: Table {table_info['source_table_name']} - inserted data source: {data_source}")
